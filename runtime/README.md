...
- `runtime/query-engine` — SPEC-004 Query Engine (finite one-shot execution over read-only `QuerySource` providers, with validation, authorization filtering, consistency-boundary recording, provenance, classification, projection, sorting, aggregation, SavedQuery catalog registration/replay, limits, and cancellation; no concrete engine adapters, Security implementation, concrete SavedQuery persistence, populated Context Packages, cursors, or parallel execution yet).
