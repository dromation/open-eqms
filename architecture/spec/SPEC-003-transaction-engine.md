...
- The Transaction Store contract exposes exactly one atomic allocate-and-append operation — not separate pre-allocation calls. Given a fully validated, and — for Level 3 — signature-verified and hash-computed Transaction ready to record, it atomically assigns the next Append Index (every level) and, for Level 2+, the next Transaction Sequence value, and durably persists the record — either independently, or staged as part of a supplied Unit of Work and finalized only at that Unit of Work's commit. A design exposing `next_append_index()` / `next_transaction_sequence()` as calls independent from the append/stage operation does not satisfy this contract: it risks a value being allocated and never associated with any durably stored record if a later step fails.
...
2. Define TransactionId, transaction schema version, Transaction Level, Append Index, Transaction Sequence, PriorReference (a tagged Transaction/Event link), PriorTransactionHash, ActorRef/DeviceRef/SiteRef (local to this crate), and the Level 1/2/3 field-set data contracts as idiomatic Rust types, reusing `ObjectId`, the shared `Version` type, `PropertyValue`, and the Unit of Work contracts from the shared contracts layer (runtime-contracts).
...
5. Implement the Append operation in this order: structural validation, then (Level 3 only) signature verification and signed-revision correspondence, then transaction hash computation via the Cryptographic Provider, then atomic allocation of Append Index (and, for Level 2+, Transaction Sequence) together with durable persistence or Unit-of-Work staging — as a single atomic Transaction Store operation, never as separate pre-allocation and append calls (see "Storage Abstraction").
...
9. Write the conformance test suite covering "Testing Requirements" and all sixteen acceptance criteria, including the Unit-of-Work commit/rollback tests using a test double for shared storage wiring.
